...
export default function Main(props) {
  ...
  useEffect(() => {
    ...
    if (category == undefined && type == undefined && searchedWord == undefined){
      ...
      db.collection('jobs').onSnapshot(snapshot => {
        ...
        let newData = snapshot.docs.map(doc=>{
          ...
          if (searchedWord!=" "){
            if (searchedCategory!="all_categories"){
              if ((docData['address'].indexOf(searchedWord) >= 0 || docData['category'].indexOf(searchedWord) >= 0 || docData['description'].indexOf(searchedWord) >= 0 || docData['location'].indexOf(searchedWord) >= 0 || docData['org_name'].indexOf(searchedWord) >= 0 || docData['salary'].indexOf(searchedWord) >= 0 || docData['title'].indexOf(searchedWord) >= 0) && docData['category']==searchedCategory ){
                docData["id"] = doc.id;
          ...
          else if (searchedCategory!="all_categories" && docData['category']==searchedCategory){
            console.log("category",firstList)
            secondList.push(docData)
            return docData
          }
          else if (searchedCity!="all_cities" && docData['city']==searchedCity){
            console.log("category",firstList)
            secondList.push(docData)
            return docData
          }
        ...
        setJobList(firstList.concat(secondList))
        console.log("firstList",firstList)
        console.log("jobList",secondList)
    ...
    <div>
      ...
      <div id="main_tab">
        <div id="main_tab-in">
          ...
          <div className="b_postajob">
            <a href="https://api.whatsapp.com/send?phone=+996772409638&text=Здраствуйте, я бы хотел добавить вакансию на сайте bagyt.ru" target="_blank">Добавить ВАКАНСИЮ</a>
          </div>
      ...
      <div className="category_list">
        <div className="category_list-in">
          <ul>
            ...
            {dbCategories.map((category,index) => {
                return (
                  <li key={index} className="cat-item  cat-item-3">
            ...
            <ul>
              <li className="widget">
                ...
                <div className="textwidget">
                  Мы агентство Bagyt.ru помогаем с легкостю найти работу или сотрудников для вашей компаний.
      ...
      <div id="footer" className="clearfix">
        <div id="footer-in">
          ...
          <a href="/admin">Copyright 2021 © Bagyt All rights reserved.</a>
        </div>
        {/*footer-in end*/}
      </div>
    </div>
