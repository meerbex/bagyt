...
export default function Change(props) {
  ...
  const [postedOn, setPosted] = useState("")
  ...
  useEffect(() => {
    db.collection('jobs').doc(props.postId).get()
    .then(snapshot => {
      ...
      setOrg_email(job.org_email);
      setOrg_name(job.org_name);
      setOrg_phone(job.org_phone);
      setSalary(job.salary);
      setTitle(job.title);
      setType(job.type);
      setPosted(job.postedOn)
      
      setImagePreviewUrl(job.imageUrl);
    });
      ...
      <div id="jobform" className="submit_job_form">
        ...
        <p className="mandatory fr"> <span className="required">*</span> означает обязательное поле </p>
          ...
          <div className="form_row clearfix">
            ...
            <select className="textfield slog_prop" defaultValue={category} name="radius" onChange={e=>setCategory(e.target.value)} required="true">
              <option  value="">Выберите категорию</option>
              {dbCategories.map((dbcategory,index) => {
                return (
                  <option key={index} value={dbcategory.name} selected={dbcategory==category?"true":"false"} >{dbcategory.name}</option>
                )
              })}
				    </select>
          </div>
            ...
            <div className="form_cat_right" style={{display: 'flex'}}>
              ...
              <div className="form_cat ">
                <label className="r_lbl" htmlFor="job_type_2">
                  <input name="job_type" checked={ type == "Неполная занятость" } id="job_type_2" type="radio" onChange={e=>setType("Неполная занятость")} defaultValue="Неполная занятость" /> Неполная занятость
                </label>
              </div>
              <div className="form_cat ">
                <label className="r_lbl" htmlFor="job_type_3">
                  <input name="job_type" checked={ type == "Удаленна работа" } id="job_type_3" type="radio" onChange={e=>setType("Удаленна работа")} defaultValue="Удаленна работа" /> Удаленна работа
                </label>
              </div>
            </div>             
          </div>
        ...
        <div name="loginform" className="sublog_prop prop_sub_login" id="login_user_frm_id" method="post">
          ...
          <button className="btn_input_normal btn_spacer" type="submit">Изменить</button>
        </div>
